...
!===================================================================================
!***********************************************************************************
!-           Set of Fortran90 functions for "AUTOSURF_rigid4D" PROGRAM             -
!***********************************************************************************
...
do j=2,3
 if(xi(j)>0.9999d0.or.xi(j)<-0.9999d0)then
   func1=0d0
   return
 endif
enddo
       ...
       do L2=0,order_3
         if((L1+L2)<order_4+1)then
           do M=0,min(L1,L2)
            ...
            grad2(1)=grad2(1)+b2(count,jj)*dble(R)*alpha*xbeta*(jac3(1))**(xbeta-1d0)*(jac4(1))**(R)*PM1(M,L1)*PM2(M,L2)* &
                     cos(dble(M)*jac4(4))
       ...
       do L2=0,order_3_min
         if((L1+L2)<order_4_min+1)then
           do M=0,min(L1,L2)
            ...
            grad2(1)=grad2(1)+b2_seed(count,jj)*dble(R)*alpha*xbeta*(jac3(1))**(xbeta-1d0)*(jac4(1))**(R)*PM1(M,L1)*PM2(M,L2)* &
                     cos(dble(M)*jac4(4))
       ...
       do L2=0,order_3
         if((L1+L2)<order_4+1)then
           do M=0,min(L1,L2)
            ...
            grad2(1)=grad2(1)+b2_lower(count,jj)*dble(R)*alpha*xbeta*(jac3(1))**(xbeta-1d0)*(jac4(1))**(R)*PM1(M,L1)*PM2(M,L2)* &
                     cos(dble(M)*jac4(4))
